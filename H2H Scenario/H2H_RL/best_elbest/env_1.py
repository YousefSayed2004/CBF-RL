...
class RewardConfig:
    ...
    deviation_weight: float = 0.3/100
...
class TwoVehicleLambdaEnv:
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        obs = np.array([
            self.x_i[0], self.x_i[1], self.x_i[2], self.x_i[3],
            self.x_j[0], self.x_j[1], self.x_j[2], self.x_j[3],
            self.u_j_prev[0], self.u_j_prev[1],
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        if infeasible_i or infeasible_j:
            ...
            current_inter_clear = min_intervehicle_clearance(
                self.x_i, self.x_j, self.vp_i, self.vp_j, self.circle_offsets, self.circle_radius
            )

            if current_inter_clear > 1.0 and current_inter_clear > self.episode_min_intervehicle_clearance:
                reward = float(0.5 * self.reward_cfg.infeasible_qp_penalty)
            else:
            ...
            info = {
                ...
                "u_i": u_i.copy(),
                "u_j": u_j.copy(),
                "inter_vehicle_clearance": current_inter_clear,
        ...
        if(inter_clear >= 0.5 and inter_clear > self.episode_min_intervehicle_clearance):   
            reward += self.reward_cfg.survival_bonus * 2
        ...
        if(moved_i >= 7.5 and moved_j >= 7.5 and inter_clear <= self.episode_min_intervehicle_clearance):
            if(inter_clear <= 11.0 and inter_clear >= 6.0):
                reward += abs(self.u_i_prev[0] - self.u_j_prev[0]) * self.reward_cfg.high_speed_reward_weight
            elif(inter_clear < 6.0 and (self.u_i_prev[0] == self.u_j_prev[0])):
                reward += self.reward_cfg.high_speed_reward_weight * 2
        # if(inter_clear >= 0.5 and inter_clear > self.episode_min_intervehicle_clearance):   
        #     if(self.u_i_prev[0] >= 0.0):
        #         reward += self.u_i_prev[0] * self.reward_cfg.high_speed_reward_weight
        #     if(self.u_j_prev[0] >= 0.0):
        #         reward += self.u_j_prev[0] * self.reward_cfg.high_speed_reward_weight

        collision = inter_clear < -1e-3
        boundary_collision = (bound_i < -1e-3) or (bound_j < -1e-3)
        ...
        if collision:
            reward = float(self.reward_cfg.collision_penalty +
                0.5 * self.reward_cfg.collision_penalty * max(self.lambda_i, self.lambda_j))
            if(min(moved_i, moved_j) <= 6.5): 
                reward *= 2.5 
            ...
            early_boundary_collision = (
                (max(moved_i, moved_j) < 15.0 and inter_clear >= 3.0) or (min(moved_i, moved_j) <= 6.5)
            )
            ...
            if early_boundary_collision:
                ...
                reward = float(self.reward_cfg.boundary_collision_penalty +
                               0.5 * self.reward_cfg.boundary_collision_penalty * max(self.lambda_j, self.lambda_i))
