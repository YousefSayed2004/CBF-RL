...
class TwoVehicleLambdaEnv:
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        #if inter_clear < self.reward_cfg.collision_threshold:
        #    reward -= max(self.x_i[3], self.x_j[3]) * self.reward_cfg.high_speed_penalty_weight
        ...
        if collision:
            reward = float(self.reward_cfg.collision_penalty)
                #0.25 * self.reward_cfg.collision_penalty * max(self.lambda_i, self.lambda_j))
            ...
            early_boundary_collision = (
                max(moved_i, moved_j) < 15.0 and inter_clear >= 3.0
            )
